...
export default function Acceuil() {
  ...
  const eventsScrollRef   = useRef(null); // ← new
  ...
  return (
    <>
      <Head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Syne:wght@400;700;800&family=Inter:wght@300;400;500;600;700&family=Amiri:ital,wght@0,400;0,700;1,400;1,700&family=Cairo:wght@400;600;700;800&display=swap" rel="stylesheet" />
      ...
      <style jsx global>{`
        *, *::before, *::after { box-sizing:border-box; }
        ...
        /* Scroll card — shared between Cities / Stadiums / Events */
        .scroll-card { flex-shrink:0; width:300px; height:400px; border-radius:16px; overflow:hidden; position:relative; cursor:pointer; transition:border-color .3s, transform .3s, box-shadow .3s; }

        /* Event modal */
        .modal-backdrop { position:fixed;inset:0;z-index:900;background:rgba(0,0,0,.72);display:flex;align-items:center;justify-content:center;padding:20px;animation:bgIn .25s ease; }
        .modal-card { background:#fff;border-radius:24px;overflow:hidden;width:100%;max-width:640px;max-height:90vh;overflow-y:auto;animation:modalIn .35s cubic-bezier(.22,.68,0,1.2); box-shadow:0 40px 100px rgba(0,0,0,.4); }
          ...
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',maxWidth:900,margin:'0 auto'}}>
            <div className="anim-fadeUp" style={{marginBottom:40,background:'rgba(255,255,255,.07)',backdropFilter:'blur(16px)',border:'1px solid rgba(255,255,255,.14)',borderRadius:20,padding:'18px 32px'}}>
              ...
              <div id="countdown" style={{display:'flex',alignItems:'center',gap:20}}>
                {[['days','Days'],['hours','Hrs'],['minutes','Min'],['seconds','Sec']].map(([id,label],i)=>(
            ...
            <div className="anim-fadeUp d3" style={{display:'flex',gap:12,flexWrap:'wrap',justifyContent:'center'}}>
              <button onClick={()=>router.push('/matches')} style={{display:'flex',alignItems:'center',gap:8,padding:'13px 28px',background:'linear-gradient(135deg,#C1272D,#a01e23)',color:'#fff',borderRadius:12,border:'none',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:14,cursor:'pointer',boxShadow:'0 8px 24px rgba(193,39,45,.35)',transition:'all .25s'}}
              ...
              <button onClick={()=>router.push('/cities')} style={{display:'flex',alignItems:'center',gap:8,padding:'13px 28px',background:'rgba(255,255,255,.1)',color:'#fff',borderRadius:12,border:'1.5px solid rgba(255,255,255,.3)',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:14,cursor:'pointer',backdropFilter:'blur(12px)',transition:'all .25s'}}
      ...
      <section id="matches" style={{padding:'80px 0',background:'#fff',borderBottom:'1px solid #e7e5e4'}}>
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px'}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:36,flexWrap:'wrap',gap:16}}>
            ...
            <a onClick={()=>router.push('/matches')} style={{display:'flex',alignItems:'center',gap:6,fontSize:13,fontWeight:700,color:'#C1272D',fontFamily:'Syne,sans-serif',textDecoration:'none',cursor:'pointer'}}>
              Full Calendar<span className="material-icons" style={{fontSize:16}}>arrow_forward</span>
            </a>
          </div>
          {upcomingMatches.length===0 ? (
            <p style={{textAlign:'center',padding:'48px 0',color:'#a8a29e'}}>No upcoming matches at the moment.</p>
          ) : (
      ...
      <section id="cities" style={{padding:'64px 0',background:'linear-gradient(135deg,#2d0a0e,#1a0608,rgba(0,98,51,.3))',position:'relative',overflow:'hidden',borderBottom:'1px solid rgba(255,255,255,.05)'}}>
        ...
        {/* Header */}
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:28,position:'relative',zIndex:2}}>
          <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div style={{display:'flex',gap:8}}>
            {['arrow_back','arrow_forward'].map((icon,i)=>(
              <button key={icon} onClick={()=>scroll(citiesScrollRef,i===0?'left':'right')}
                style={{width:40,height:40,borderRadius:'50%',border:'1.5px solid rgba(255,255,255,.2)',background:'rgba(255,255,255,.05)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'background .2s'}}
        ...
        <div ref={citiesScrollRef} className="no-scrollbar" style={{display:'flex',overflowX:'auto',gap:16,paddingBottom:4,paddingLeft:24,paddingRight:24,position:'relative',zIndex:2}}>
          {cities.map(city=>(
            <div key={city.id} className="scroll-card" style={{border:'1.5px solid rgba(255,255,255,.08)'}}
      ...
      <section id="stades" style={{padding:'64px 0',background:'#fff',borderBottom:'1px solid #e7e5e4',overflow:'hidden',position:'relative'}}>
        ...
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:28,position:'relative',zIndex:2}}>
          ...
          <div style={{display:'flex',gap:8}}>
            {['arrow_back','arrow_forward'].map((icon,i)=>(
              <button key={icon} onClick={()=>scroll(stadesScrollRef,i===0?'left':'right')}
                style={{width:40,height:40,borderRadius:'50%',border:'1.5px solid #e7e5e4',background:'#fff',color:'#57534e',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .2s'}}
        ...
        <div ref={stadesScrollRef} className="no-scrollbar" style={{display:'flex',overflowX:'auto',gap:16,paddingBottom:4,paddingLeft:24,paddingRight:24,position:'relative',zIndex:2}}>
          {stades.map(stade=>(
            <div key={stade.id} className="scroll-card" style={{border:'1.5px solid #e7e5e4'}}
        ...
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:14}}>
              <div style={{display:'flex',gap:5,alignItems:'center'}}>
                {latestNews.map((_,idx)=>(
                  <button key={idx} onClick={()=>setCurrentNewsIndex(idx)} style={{width:idx===currentNewsIndex?24:8,height:8,borderRadius:999,background:idx===currentNewsIndex?'#C1272D':'#e7e5e4',border:'none',cursor:'pointer',transition:'all .35s',padding:0}} />
                ))}
              </div>
              <div style={{display:'flex',gap:6}}>
                {['arrow_back','arrow_forward'].map((icon,i)=>(
                  <button key={icon} onClick={()=>scroll(newsScrollRef,i===0?'left':'right',400)}
                    style={{width:38,height:38,borderRadius:'50%',border:'1.5px solid #e7e5e4',background:i===1?'linear-gradient(135deg,#C1272D,#a01e23)':'#fff',color:i===1?'#fff':'#57534e',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .2s'}}
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 310px',gap:28,alignItems:'start'}}>
            {latestNews.length>0 && latestNews[currentNewsIndex] && (
              <article key={`f-${currentNewsIndex}`} className="news-card" 
                onClick={()=>router.push(`/news?id=${latestNews[currentNewsIndex].id}`)}
                style={{cursor:'pointer',animation:newsVisible?'fadeLeft .65s cubic-bezier(.22,.68,0,1.2) both':'none'}}>
                <div style={{position:'relative',height:420,borderRadius:16,overflow:'hidden',marginBottom:20}}>
                  ...
                  <img className="news-card-img" src={latestNews[currentNewsIndex].imageUrl||''} alt={latestNews[currentNewsIndex].title} style={{width:'100%',height:'100%',objectFit:'cover'}} />
                  <div style={{position:'absolute',top:20,left:20,zIndex:2}}>
                    <span style={{display:'inline-flex',alignItems:'center',padding:'5px 12px',background:'linear-gradient(135deg,#C1272D,#a01e23)',color:'#fff',borderRadius:999,fontSize:10,fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',fontFamily:'Syne,sans-serif'}}>
                      {latestNews[currentNewsIndex].category||'NEWS'}
                    </span>
                  </div>
                  <div style={{position:'absolute',top:20,right:20,zIndex:2,display:'flex',gap:5}}>
                    {latestNews.map((_,idx)=>(
                      <div key={idx} onClick={e=>{e.stopPropagation();setCurrentNewsIndex(idx);}} style={{width:idx===currentNewsIndex?24:8,height:8,borderRadius:999,background:idx===currentNewsIndex?'#fff':'rgba(255,255,255,.35)',cursor:'pointer',transition:'all .35s'}} />
                    ))}
                  </div>
                  <div style={{position:'absolute',bottom:24,left:24,right:24,zIndex:2}}>
                    <h3 style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(18px,2.4vw,26px)',color:'#fff',lineHeight:1.25,marginBottom:8}}>
                      {latestNews[currentNewsIndex].title||'Latest News'}
                    </h3>
                    <div style={{display:'flex',alignItems:'center',gap:8,fontSize:11,color:'rgba(255,255,255,.5)'}}>
                ...
                <p style={{fontFamily:'Inter,sans-serif',fontSize:14,color:'#78716c',lineHeight:1.75}}>{latestNews[currentNewsIndex].description||''}</p>
              </article>
            )}
            ...
            <div style={{display:'flex',flexDirection:'column',gap:0,animation:newsVisible?'fadeRight .65s cubic-bezier(.22,.68,0,1.2) .1s both':'none'}}>
              ...
              {getSideNews().map((news,i)=>(
                <article key={news.id||i} 
                  onClick={()=>router.push(`/news?id=${news.id}`)}
                  style={{display:'flex',gap:12,alignItems:'flex-start',padding:'14px 4px',borderBottom:i<2?'1px solid #f5f5f4':'none',cursor:'pointer',borderRadius:8,transition:'background .2s'}}
                  onMouseEnter={e=>e.currentTarget.style.background='#fafaf9'}
                  onMouseLeave={e=>e.currentTarget.style.background=''}>
                  <div style={{width:68,height:58,borderRadius:10,overflow:'hidden',flexShrink:0,border:'1px solid #e7e5e4'}}>
                    <img style={{width:'100%',height:'100%',objectFit:'cover',transition:'transform .5s'}} src={news.imageUrl||''} alt={news.title}
                      onMouseEnter={e=>e.target.style.transform='scale(1.08)'}
                      onMouseLeave={e=>e.target.style.transform=''} />
                  </div>
                  <div style={{flex:1}}>
                    <div style={{fontSize:9,fontWeight:700,letterSpacing:'.08em',textTransform:'uppercase',fontFamily:'Syne,sans-serif',color:i===0?'#006233':i===1?'#b07d00':'#C1272D',marginBottom:5}}>{news.category||'NEWS'}</div>
                    <h4 style={{fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,color:'#1c1917',lineHeight:1.45,marginBottom:5}}>{news.title||'News'}</h4>
              ...
              <div style={{marginTop:18,padding:18,background:'linear-gradient(135deg,#2d0a0e,#1a0608)',borderRadius:14,position:'relative',overflow:'hidden',border:'1px solid rgba(193,39,45,.18)'}}>
                ...
                <div style={{position:'relative',zIndex:1}}>
                  ...
                  <button onClick={()=>router.push('/newsletter')} style={{width:'100%',padding:'9px 0',background:'linear-gradient(135deg,#C1272D,#a01e23)',color:'#fff',border:'none',borderRadius:8,fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,cursor:'pointer',transition:'opacity .2s'}}
                    onMouseEnter={e=>e.currentTarget.style.opacity='.85'}
                    onMouseLeave={e=>e.currentTarget.style.opacity='1'}>Subscribe Free →</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="culture" ref={cultureSectionRef} style={{padding:'80px 0',background:'linear-gradient(135deg,#2d0a0e,#1a0608,rgba(0,55,28,.5))',position:'relative',overflow:'hidden',borderBottom:'1px solid rgba(255,255,255,.04)'}}>
        ...
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',position:'relative',zIndex:2}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-end',marginBottom:36,flexWrap:'wrap',gap:16}}>
            ...
            <button onClick={()=>router.push('/culture')}
              style={{display:'flex',alignItems:'center',gap:8,padding:'9px 18px',background:'rgba(255,255,255,.07)',border:'1px solid rgba(255,255,255,.14)',borderRadius:10,color:'rgba(255,255,255,.7)',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,cursor:'pointer',transition:'all .2s'}}
          ...
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gridTemplateRows:'260px 260px',gap:12,height:532}}>
            {cultures[0] && (
              <div className="culture-card" style={{gridRow:'1/3',border:'1.5px solid rgba(255,255,255,.07)',animation:cultureVisible?'scaleIn .7s cubic-bezier(.22,.68,0,1.2) .05s both':'none'}}
                onClick={()=>router.push(`/culture?id=${cultures[0].id}`)}
                onMouseEnter={()=>setHoveredCulture(0)} onMouseLeave={()=>setHoveredCulture(null)}>
                <div className="culture-ovl" /><img className="culture-img" src={cultures[0].imageUrl||''} alt={cultures[0].title} />
                <div className="culture-body">
                  <div className="culture-tag">{cultures[0].category||'CULTURE'}</div>
                  <h3 className="culture-title" style={{fontSize:'clamp(20px,2.4vw,30px)'}}>{cultures[0].title||''}</h3>
                  <div className="culture-cta"><span style={{fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:10,color:'rgba(255,255,255,.6)',letterSpacing:'.08em'}}>EXPLORE</span><span className="material-icons" style={{fontSize:15,color:'#f0a500'}}>arrow_forward</span></div>
                </div>
              </div>
            )}
            {cultures[1] && (
              <div className="culture-card" style={{border:'1.5px solid rgba(255,255,255,.07)',animation:cultureVisible?'scaleIn .7s cubic-bezier(.22,.68,0,1.2) .15s both':'none'}}
                onClick={()=>router.push(`/culture?id=${cultures[1].id}`)}
                onMouseEnter={()=>setHoveredCulture(1)} onMouseLeave={()=>setHoveredCulture(null)}>
                <div className="culture-ovl" /><img className="culture-img" src={cultures[1].imageUrl||''} alt={cultures[1].title} />
                <div className="culture-body"><div className="culture-tag">{cultures[1].category||'HERITAGE'}</div><h4 className="culture-title" style={{fontSize:16}}>{cultures[1].title||''}</h4></div>
              </div>
            )}
            {cultures[2] && (
              <div className="culture-card" style={{border:'1.5px solid rgba(255,255,255,.07)',animation:cultureVisible?'scaleIn .7s cubic-bezier(.22,.68,0,1.2) .25s both':'none'}}
                onClick={()=>router.push(`/culture?id=${cultures[2].id}`)}
                onMouseEnter={()=>setHoveredCulture(2)} onMouseLeave={()=>setHoveredCulture(null)}>
                <div className="culture-ovl" /><img className="culture-img" src={cultures[2].imageUrl||''} alt={cultures[2].title} />
                <div className="culture-body"><div className="culture-tag">{cultures[2].category||'ARCHITECTURE'}</div><h4 className="culture-title" style={{fontSize:16}}>{cultures[2].title||''}</h4></div>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ══ EVENTS — horizontal scroll (same pattern as Cities) ══════════════ */}
      <section id="events" style={{padding:'64px 0',background:'#fafaf9',borderBottom:'1px solid #e7e5e4',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{maxWidth:1280,margin:'0 auto',padding:'0 24px',display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:28,position:'relative',zIndex:2}}>
          <div>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:4}}>
              <div className="accent-bar" />
              <h2 style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(22px,3vw,32px)',color:'#1c1917',letterSpacing:'-.02em',lineHeight:1.1}}>
                Beyond the <span style={{fontFamily:'Amiri,serif',fontStyle:'italic',fontWeight:700,color:'#C1272D'}}>Stadiums</span>
              </h2>
            </div>
            <p style={{fontFamily:'Inter,sans-serif',fontSize:13,color:'#78716c',marginLeft:13}}>Fanzones, festivals, and celebrations across the Kingdom</p>
          </div>
          <div style={{display:'flex',gap:8,flexShrink:0}}>
            {['arrow_back','arrow_forward'].map((icon,i)=>(
              <button key={icon} onClick={()=>scroll(eventsScrollRef,i===0?'left':'right')}
                style={{width:40,height:40,borderRadius:'50%',border:'1.5px solid #e7e5e4',background:'#fff',color:'#57534e',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'all .2s'}}
                onMouseEnter={e=>{e.currentTarget.style.borderColor='#C1272D';e.currentTarget.style.color='#C1272D';}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor='#e7e5e4';e.currentTarget.style.color='#57534e';}}>
                <span className="material-icons" style={{fontSize:20}}>{icon}</span>
        ...
        {/* Scroll track — same structure as Cities / Stadiums */}
        <div ref={eventsScrollRef} className="no-scrollbar" style={{display:'flex',overflowX:'auto',gap:16,paddingBottom:4,paddingLeft:24,paddingRight:24,position:'relative',zIndex:2}}>
          {upcomingEvents.map((event,ei)=>(
            <div key={event.id}
              className="scroll-card"
              style={{border:'1.5px solid #e7e5e4', background:'#fff'}}
              onClick={()=>setSelectedEvent(event)}
              onMouseEnter={e=>{e.currentTarget.style.borderColor='#C1272D';e.currentTarget.style.transform='translateY(-4px)';e.currentTarget.style.boxShadow='0 20px 48px rgba(193,39,45,.12)';}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor='#e7e5e4';e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='';}}>
              {/* Image */}
              <div style={{position:'absolute',inset:0,background:'linear-gradient(to top,rgba(0,0,0,.82),rgba(0,0,0,.2) 55%,transparent)',zIndex:1}} />
              <img src={event.imageUrl||''} alt={event.name}
                ...
                onMouseLeave={e=>e.target.style.transform=''} />
              {/* Badges */}
              <div style={{position:'absolute',top:14,left:14,zIndex:2}}><span className="pill-dark">Event</span></div>
              {event.priceProxim!=null && (
                <div style={{position:'absolute',top:14,right:14,zIndex:2,background:'rgba(0,0,0,.5)',backdropFilter:'blur(8px)',border:'1px solid rgba(255,255,255,.18)',borderRadius:999,padding:'4px 12px'}}>
                  <span style={{fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:10,color:'#f0a500',letterSpacing:'.06em'}}>{event.priceProxim===0?'Free':`${event.priceProxim} MAD`}</span>
                </div>
              )}
              {/* Info */}
              <div style={{position:'absolute',bottom:0,left:0,right:0,padding:24,zIndex:2}}>
                <div style={{display:'flex',alignItems:'center',gap:5,fontSize:10,color:'rgba(255,255,255,.55)',fontFamily:'Inter,sans-serif',marginBottom:6}}>
                  <span className="material-icons" style={{fontSize:12}}>calendar_today</span>
                  {new Date(event.dateOfEvent).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'})}
                </div>
                <h3 style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:18,color:'#fff',lineHeight:1.2,marginBottom:6}}>{event.name||'Event'}</h3>
                <p style={{fontFamily:'Inter,sans-serif',fontSize:11,color:'rgba(255,255,255,.55)',display:'flex',alignItems:'center',gap:4}}>
                  <span className="material-icons" style={{fontSize:12}}>location_on</span>{event.cityName||'Location TBD'}
                </p>
              </div>
            </div>
          ))}
      ...
      {selectedEvent && (
        <div className="modal-backdrop" onClick={()=>setSelectedEvent(null)}>
          <div className="modal-card" onClick={e=>e.stopPropagation()}>
            ...
            {/* Hero image */}
            <div style={{position:'relative',height:260,background:'#f5f5f4',flexShrink:0}}>
              ...
              <button onClick={()=>setSelectedEvent(null)}
                style={{position:'absolute',top:14,right:14,width:38,height:38,borderRadius:'50%',background:'rgba(0,0,0,.4)',backdropFilter:'blur(8px)',border:'1px solid rgba(255,255,255,.2)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',transition:'background .2s',zIndex:10}}
              ...
              {/* Price badge */}
              {selectedEvent.priceProxim!=null && (
                <div style={{position:'absolute',top:14,left:14,padding:'5px 14px',background:'rgba(0,0,0,.5)',backdropFilter:'blur(8px)',border:'1px solid rgba(255,255,255,.18)',borderRadius:999,fontFamily:'Syne,sans-serif',fontSize:11,fontWeight:800,color:'#f0a500'}}>
                  {selectedEvent.priceProxim===0?'Free Entry':`${selectedEvent.priceProxim} MAD`}
                </div>
              )}

              {/* Title on image */}
              <div style={{position:'absolute',bottom:22,left:24,right:24,zIndex:2}}>
                <span style={{display:'inline-flex',alignItems:'center',padding:'3px 10px',background:'rgba(193,39,45,.85)',color:'#fff',borderRadius:999,fontSize:9,fontWeight:700,letterSpacing:'.1em',textTransform:'uppercase',fontFamily:'Syne,sans-serif',marginBottom:8}}>
                  Event
                </span>
                <h2 style={{fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:'clamp(20px,3vw,26px)',color:'#fff',lineHeight:1.15}}>
                  {selectedEvent.name}
                </h2>
              </div>
            </div>
            ...
            <div style={{padding:'28px 28px 32px'}}>
              ...
              <div style={{display:'flex',gap:12,flexWrap:'wrap',marginBottom:24}}>
                {[
                  {icon:'calendar_today', val:new Date(selectedEvent.dateOfEvent).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'}), color:'#C1272D'},
                  {icon:'location_on',    val:selectedEvent.cityName||'Location TBD',  color:'#006233'},
                ].map((m,i)=>(
                  <div key={i} style={{display:'flex',alignItems:'center',gap:8,padding:'8px 14px',background:'#fafaf9',border:'1px solid #e7e5e4',borderRadius:12,flex:1,minWidth:160}}>
                    <div style={{width:32,height:32,borderRadius:10,background:`${m.color}12`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                      <span className="material-icons" style={{fontSize:16,color:m.color}}>{m.icon}</span>
                    </div>
                    <span style={{fontFamily:'Inter,sans-serif',fontSize:13,color:'#1c1917',fontWeight:500,lineHeight:1.3}}>{m.val}</span>
                  </div>
                ))}
              </div>
              ...
              {selectedEvent.description && (
                <div style={{marginBottom:24}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                    <div style={{width:3,height:16,background:'#C1272D',borderRadius:2}} />
                    <span style={{fontFamily:'Syne,sans-serif',fontSize:10,fontWeight:700,textTransform:'uppercase',letterSpacing:'.1em',color:'#a8a29e'}}>About this event</span>
                  </div>
                  <p style={{fontFamily:'Inter,sans-serif',fontSize:14,color:'#57534e',lineHeight:1.78}}>
                    {selectedEvent.description}
                  </p>
                </div>
              )}
              ...
              {/* CTA button */}
              <button
                onClick={()=>router.push(`/events?id=${selectedEvent.id}`)}
                style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'13px 0',background:'linear-gradient(135deg,#C1272D,#ea580c)',color:'#fff',border:'none',borderRadius:12,fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:14,cursor:'pointer',transition:'all .25s',boxShadow:'0 6px 20px rgba(193,39,45,.3)'}}
                onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 12px 32px rgba(193,39,45,.4)';}}
                onMouseLeave={e=>{e.currentTarget.style.transform='';e.currentTarget.style.boxShadow='0 6px 20px rgba(193,39,45,.3)';}}>
