...
const MATCH_TYPES  = ["Group stage","Round of 16","Quarter-final","Semi-final","Third place","Final"];

const statusCfg = s => STATUS_CONFIG[normStatus(s)] || STATUS_CONFIG.SCHEDULED;
const isLive    = s => normStatus(s) === "LIVE";

const EMPTY_ADD = { dateOfMatch:"", stadeId:"", referee:"", statut:"SCHEDULED", type:"Group stage" };
...
export default function MatchesRespo() {
  ...
  const [deleteTarget,   setDeleteTarget]   = useState(null); // match object
  ...
  const loadStades = () => {
    fetch(`${BASE}/acceuil/stade/all`)
      .then(r => r.json())
      .then(d => setStades(Array.isArray(d)?d:[]))
      .catch(() => {});
  };

  useEffect(() => { loadMatches(); loadStades(); }, []);
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      f = f.filter(m =>
        m.matchTeams?.[0]?.teamName?.toLowerCase().includes(q) ||
        m.matchTeams?.[1]?.teamName?.toLowerCase().includes(q) ||
        m.stadeName?.toLowerCase().includes(q)
      );
    }
    if (filterStatus !== "ALL") f = f.filter(m => normStatus(m.statut) === filterStatus);
    setFilteredMatches(f);
  }, [search, filterStatus, matches]);

  /* ── ADD MATCH ── */
  const submitAdd = async () => {
    if (!addForm.dateOfMatch || !addForm.stadeId) {
      showToast("error","Date et stade sont requis."); return;
    }
    setSavingAdd(true);
    try {
      const body = {
        dateOfMatch: addForm.dateOfMatch,
        referee:     addForm.referee || null,
        status:      addForm.statut,
        type:        addForm.type,
        stade:       { id: parseInt(addForm.stadeId) },
      };
      const res = await fetch(`${BASE}/matches/matches/add`, {
      ...
      if (res.ok) {
        ...
        loadMatches();
      } else showToast("error","Échec ajout match.");
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`${BASE}/matches/matches/delete/${deleteTarget.id}`, { method:"DELETE" });
      if (res.ok) {
        showToast("success","Match supprimé !");
        setMatches(p => p.filter(m => m.id !== deleteTarget.id));
  ...
  return (
    <>
      <Head>
        <title>Matches · MoroccoFan2030</title>
      ...
      <style jsx global>{`
        *,*::before,*::after{box-sizing:border-box}
        ...
        .mr-title-row{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;}
        ...
        .mr-toolbar{display:flex;align-items:center;gap:8px;padding-bottom:14px;flex-wrap:wrap;}
        .mr-search-wrap{position:relative;flex:1;min-width:180px;}
        .mr-search-wrap .mi{position:absolute;left:10px;top:50%;transform:translateY(-50%);font-size:15px;color:rgba(255,255,255,.28);pointer-events:none;}
        .mr-search{width:100%;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.09);border-radius:9px;color:#fff;font-family:'Inter',sans-serif;font-size:13px;padding:8px 12px 8px 34px;outline:none;transition:border-color .2s;}
        .mr-search:focus{border-color:rgba(193,39,45,.4);}
        .mr-search::placeholder{color:rgba(255,255,255,.22);}

        .mr-filter-btn{padding:7px 13px;border-radius:8px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);color:rgba(255,255,255,.5);font-family:'Syne',sans-serif;font-size:11px;font-weight:700;cursor:pointer;transition:all .18s;white-space:nowrap;}
        ...
        /* table */
        .mr-table{width:100%;background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.07);border-radius:14px;overflow:hidden;}
        .mr-thead{display:grid;grid-template-columns:1fr 80px 1fr 160px 120px 80px;gap:8px;padding:10px 18px;border-bottom:1px solid rgba(255,255,255,.06);font-family:'Syne',sans-serif;font-size:10px;font-weight:700;color:rgba(255,255,255,.28);text-transform:uppercase;letter-spacing:.08em;}
        .mr-row{display:grid;grid-template-columns:1fr 80px 1fr 160px 120px 80px;gap:8px;padding:13px 18px;border-bottom:1px solid rgba(255,255,255,.04);align-items:center;cursor:pointer;transition:background .15s;}
        ...
        .team-logo{width:28px;height:28px;border-radius:6px;object-fit:cover;flex-shrink:0;}
        .team-logo-placeholder{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;font-family:'Syne',sans-serif;font-weight:800;font-size:10px;color:#fff;flex-shrink:0;}
        .team-name{font-family:'Syne',sans-serif;font-weight:700;font-size:12px;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
        .score-cell{font-family:'Syne',sans-serif;font-weight:900;font-size:15px;color:#C1272D;text-align:center;letter-spacing:1px;}
        ...
        /* overlays */
        .mr-overlay{position:fixed;inset:0;background:rgba(0,0,0,.78);backdrop-filter:blur(6px);z-index:200;display:flex;align-items:center;justify-content:center;padding:20px;}

        /* detail modal */
        .mr-modal{background:linear-gradient(160deg,#170818 0%,#0c030f 100%);border:1px solid rgba(255,255,255,.1);border-radius:20px;width:100%;max-width:680px;max-height:90vh;overflow-y:auto;animation:scaleIn .22s ease both;}
        .mr-modal::-webkit-scrollbar{width:4px;}
        .mr-modal::-webkit-scrollbar-thumb{background:rgba(193,39,45,.3);border-radius:2px;}
        .mr-modal-head{padding:22px 24px 0;position:sticky;top:0;z-index:10;background:linear-gradient(160deg,#170818,#0c030f);border-bottom:1px solid rgba(255,255,255,.06);}
        .mr-modal-body{padding:20px 24px 24px;}
        .mr-score-hero{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:14px;padding:18px 24px;display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:18px;}
        .mr-hero-score{font-family:'Syne',sans-serif;font-weight:900;font-size:38px;color:#C1272D;letter-spacing:4px;}
        .mr-tabs{display:flex;gap:0;border-bottom:1px solid rgba(255,255,255,.07);margin-bottom:18px;}
        .mr-tab{padding:10px 16px;font-family:'Syne',sans-serif;font-weight:700;font-size:11px;color:rgba(255,255,255,.4);cursor:pointer;text-transform:uppercase;letter-spacing:.07em;border-bottom:2px solid transparent;transition:all .18s;display:flex;align-items:center;gap:6px;}
        ...
        .spill{padding:8px 14px;border-radius:9px;border:1px solid;font-family:'Syne',sans-serif;font-size:11px;font-weight:700;letter-spacing:.05em;text-transform:uppercase;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center;gap:6px;}
        .spill:hover{transform:translateY(-1px);}
        .spill:disabled{opacity:.45;cursor:not-allowed;transform:none;}

        .ev-item{display:flex;align-items:center;gap:10px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.04);}
        .ev-item:last-child{border-bottom:none;}

        @media(max-width:640px){
          .mr-thead,.mr-row{grid-template-columns:1fr 60px 1fr 100px 60px;}
          ...
          <div className="mr-title-row">
            ...
            <div style={{display:"flex",gap:8}}>
              <button className="btn-p" style={{padding:"7px 14px",fontSize:11}} onClick={()=>setShowAdd(true)}>
                <span className="material-icons" style={{fontSize:14}}>add</span>Ajouter un match
              </button>
              <button className="btn-s" style={{padding:"7px 14px",fontSize:11}} onClick={loadMatches}>
                ...
                return (
                  <div key={m.id||i} className={`mr-row mr-up${live?" live":""}`}
                    ...
                    <div className="team-cell">
                      {t1.imageUrl
                        ?<img src={t1.imageUrl} alt={t1.teamName} className="team-logo"/>
                        :<div className="team-logo-placeholder" style={{background:hue(t1.teamName)}}>{initials(t1.teamName)}</div>}
                      <span className="team-name">{t1.teamName||"—"}</span>
                    </div>

                    {/* score */}
                    <div className="score-cell">{t1.goals??0} — {t2.goals??0}</div>

                    {/* team 2 */}
                    <div className="team-cell">
                      {t2.imageUrl
                        ?<img src={t2.imageUrl} alt={t2.teamName} className="team-logo"/>
                        :<div className="team-logo-placeholder" style={{background:hue(t2.teamName)}}>{initials(t2.teamName)}</div>}
                      <span className="team-name">{t2.teamName||"—"}</span>
                    </div>
                    ...
                    <div className="row-actions" style={{justifyContent:"center"}} onClick={e=>e.stopPropagation()}>
                      <div className="settings-btn" title="Gérer" onClick={()=>openMatch(m)}>
                        <span className="material-icons" style={{fontSize:14}}>settings</span>
                      </div>
                      <div className="delete-btn" title="Supprimer" onClick={()=>setDeleteTarget(m)}>
      ...
      {/* ═══════════════════ MODAL: ADD MATCH ═══════════════════ */}
      {showAdd && (
        <div className="mr-overlay" onClick={e=>{if(e.target===e.currentTarget){setShowAdd(false);setAddForm(EMPTY_ADD);}}}>
          <div className="sm-modal">
            {/* header */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22}}>
              <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:17,color:"#fff"}}>Ajouter un match</div>
                <div style={{fontSize:11,color:"rgba(255,255,255,.3)",marginTop:2}}>Remplissez les informations du nouveau match</div>
              </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
              <div className="db-field" style={{gridColumn:"1/-1"}}>
                ...
                <input type="datetime-local" value={addForm.dateOfMatch}
                  onChange={e=>setAddForm(f=>({...f,dateOfMatch:e.target.value}))}/>
              </div>

              <div className="db-field">
                <label>Stade</label>
                <select value={addForm.stadeId} onChange={e=>setAddForm(f=>({...f,stadeId:e.target.value}))}>
                  <option value="">— Sélectionner —</option>
                  {stades.map(s=><option key={s.id} value={s.id}>{s.name} {s.cityName?`(${s.cityName})`:""}</option>)}
                </select>
              </div>
              ...
              <div className="db-field">
                <label>Arbitre (optionnel)</label>
                <input type="text" value={addForm.referee}
                  onChange={e=>setAddForm(f=>({...f,referee:e.target.value}))}
                  placeholder="Nom de l'arbitre"/>
              </div>

              <div className="db-field">
                <label>Statut initial</label>
                <select value={addForm.statut} onChange={e=>setAddForm(f=>({...f,statut:e.target.value}))}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:6}}>
              ...
              <button className="btn-p" onClick={submitAdd} disabled={savingAdd}>
                {savingAdd
                  ? <><div className="mr-spin" style={{width:13,height:13,border:"2px solid rgba(255,255,255,.3)",borderTopColor:"#fff",borderRadius:"50%"}}/>Enregistrement…</>
                  : <><span className="material-icons" style={{fontSize:14}}>add</span>Créer le match</>}
              </button>
            </div>
          </div>
        </div>
      ...
      {deleteTarget && (
        <div className="mr-overlay" onClick={e=>{if(e.target===e.currentTarget)setDeleteTarget(null);}}>
          <div className="sm-modal" style={{maxWidth:420}}>
            ...
            <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:10,padding:"12px 14px",marginBottom:22,display:"flex",alignItems:"center",gap:12}}>
              ...
              <div>
                <div style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:13,color:"#fff"}}>
                  {deleteTarget.matchTeams?.[0]?.teamName||"Équipe 1"} vs {deleteTarget.matchTeams?.[1]?.teamName||"Équipe 2"}
                </div>
                <div style={{fontSize:11,color:"rgba(255,255,255,.3)",marginTop:2}}>
                  {deleteTarget.stadeName||"—"} · #{deleteTarget.id}
                </div>
              </div>
            </div>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              ...
              <button className="btn-danger" onClick={confirmDelete} disabled={deletingMatch}>
                {deletingMatch
                  ? <><div className="mr-spin" style={{width:13,height:13,border:"2px solid rgba(239,68,68,.4)",borderTopColor:"#ef4444",borderRadius:"50%"}}/>Suppression…</>
            ...
            <div className="mr-modal-body">
              ...
              {modalTab==="status" && (
                <div>
                  ...
                  {savingStatus&&<div style={{marginTop:12,fontSize:12,color:"rgba(255,255,255,.4)",display:"flex",alignItems:"center",gap:8}}><div className="mr-spin" style={{width:12,height:12,border:"2px solid rgba(193,39,45,.4)",borderTopColor:"#C1272D",borderRadius:"50%"}}/>Mise à jour…</div>}
                </div>
              )}
              ...
              {modalTab==="events" && (
                <div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14}}>
                    <span style={{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:13,color:"#fff"}}>Événements ({matchEvents.length})</span>
                  ...
                  {showAddEvent&&(
                    <div style={{background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.08)",borderRadius:12,padding:16,marginBottom:16}}>
                      ...
                      <button className="btn-g" style={{width:"100%",justifyContent:"center"}} onClick={addEvent} disabled={savingEvent||!eventForm.teamId||!eventForm.playerId}>
                        {savingEvent?<><div className="mr-spin" style={{width:13,height:13,border:"2px solid rgba(74,222,128,.4)",borderTopColor:"#4ade80",borderRadius:"50%"}}/>…</>:<><span className="material-icons" style={{fontSize:13}}>add</span>Ajouter</>}
                      </button>
                    </div>
                  )}
                    ...
                    :<div>
                      {[...matchEvents].sort((a,b)=>(a.minute||0)-(b.minute||0)).map((ev,i)=>(
                        <div key={ev.id||i} className="ev-item">
                          ...
                          {ev.minute!=null&&<div style={{fontFamily:"'Syne',sans-serif",fontWeight:800,fontSize:12,color:"#fbbf24",marginRight:8}}>{ev.minute}'</div>}
